...
DSA_KEY=/data/local/ssh/ssh_host_dsa_key
DSA_PUB_KEY=/data/local/ssh/ssh_host_dsa_key.pub
RSA_KEY=/data/local/ssh/ssh_host_rsa_key
RSA_PUB_KEY=/data/local/ssh/ssh_host_rsa_key.pub
AUTHORIZED_KEYS=/data/local/ssh/authorized_keys
...
if [ "1" == "$DEBUG" ] ; then
    ...
    /system/bin/logwrapper /system/bin/sshd -f /data/local/ssh/sshd_config -D -d
else
    # don't daemonize - otherwise we can't stop the sshd service
    /system/bin/sshd -f /data/local/ssh/sshd_config -D
fi
